...
public interface RequestProxy {
    ...
    static RequestProxy create(String host, int port, ProxyType proxyType,
                               ...
                               String domain,
                               String workstation) {
        return new DefaultRequestProxy(host, port, proxyType, withCredential, proxyCredentialType, userName, password, domain, workstation);
    }
    ...
    static RequestProxy newCopy(RequestProxy requestProxy) {
        return new DefaultRequestProxy(requestProxy.getHost(), requestProxy.getPort(), requestProxy.getProxyType(), requestProxy.withCredential(), requestProxy.getProxyCredentialType(), requestProxy.getUserName(), requestProxy.getPassword(), requestProxy.getDomain(), requestProxy.getWorkstation());
    }
}
